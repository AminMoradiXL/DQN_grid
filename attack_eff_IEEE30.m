function [next_state, instant_out, genloss]=attack_eff_IEEE30(state, a, d)

attacked=find(state==0);
attacked=[attacked a];
[~, failed_lines, failed_times]=IEEE30(attacked);
...
if isempty(failed_times)
    ...
    ts=failed_times(length(failed_times))*1.2;
end
s=0;
attacked=find(state==0);
attacked=[attacked a];
...
if isempty(failed_times)
    s=ts*IEEE30(attacked);
else
    for i=1:length(failed_times)+1
        if i==1
            s=s+IEEE30(attacked)*failed_times(i);
        elseif i==length(failed_times)+1
            s=s+IEEE30(attacked)*(ts-failed_times(i-1));
        else
            s=s+IEEE30(attacked)*(failed_times(i)-failed_times(i-1));
        end
